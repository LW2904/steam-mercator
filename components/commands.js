...
Bot.prototype._cmdCheck = function (id, args) {
  ...
  if (args[1]) {
    ...
    .then(level => {
      ...
      this.client.chatMessage(id, `Level ${level} -> ${args[0]} would require ${data.Intermediate.Exp} EXP (${data.Intermediate.Sets} Sets) and therefore about ${data.Intermediate.Keys} Key${data.Intermediate.Keys > 1 ? 's' : ''}.`)
    }).catch(err => {
